...
export async function createTrialLesson(
  data: Omit<TrialLesson, "id" | "createdAt">
): Promise<TrialLesson> {
  ...
  await docClient.send(
    new PutCommand({
      TableName: TableName.TRIAL_LESSONS,
      Item: lesson,
    })
  );
...
export async function createGroupRegistration(
  data: Omit<GroupRegistration, "id" | "createdAt">
  ...
  await docClient.send(
    new PutCommand({
      TableName: TableName.GROUP_REGISTRATIONS,
      Item: registration,
    })
  );
...
export async function createSeminarRegistration(
  ...
  await docClient.send(
    new PutCommand({
      TableName: TableName.SEMINAR_REGISTRATIONS,
      Item: registration,
    })
  );
...
export async function createGermanRegistration(
  data: Omit<GermanRegistration, "id" | "createdAt">
): Promise<GermanRegistration> {
  const registration: GermanRegistration = {
    id: crypto.randomUUID(),
    ...data,
    createdAt: new Date().toISOString(),
  };
  ...
  await docClient.send(
    new PutCommand({
      TableName: TableName.GERMAN_REGISTRATIONS,
      Item: registration,
    })
  );
